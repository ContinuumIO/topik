...
def run_model(data_source, source_type="auto", es_index=None, tokenizer='simple', n_topics=10,
              dir_path='./topic_model', model='lda_batch', termite_plot=True, output_file=False, r_ldavis=False,
              prefix_value=None, event_value=None, content_field=None, year_field=None,
              start_year=None, stop_year=None, seed=42,
              destination_es_instance=None, destination_es_index=None, id_field=None,
              clear_es_index=False, **kwargs):
    ...
    raw_data = read_input(data_source, content_field=content_field, source_type=source_type, **kwargs)
    tokenized_text = preprocess(raw_data, tokenizer_method=tokenizer, **kwargs)
    ...
    if model == 'lda_batch':
        ...
        lda = LDA(tokenized_text, n_topics, update_every=0, passes=20)
    elif model == 'lda_online':
        # To perform lda in online mode set variables update_every, chuncksize and passes.
        lda = LDA(tokenized_text, n_topics, update_every=1, chunksize=10000, passes=1)
    else:
